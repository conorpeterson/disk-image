...
void setup() {
  ...
  trackHeight = (width / 2 - innerRadius) / tracksPerDisk;
  
  println("rad" + innerRadius + " height" + trackHeight);
...
void draw() {
  ...
  if (scanning) {
    ...
    for (int i = 0; i < tracksPerDisk; i++) {
      ...
      float innerRadius = int(0.5 * width / 3.5);
    ...
    for (int x = 0; x < tracksPerDisk; x++) {
      ...
      //float innerRadius = int(0.5 * width / 3.5);
      //float x1 = innerRadius;
      float x1 = innerRadius + x * trackHeight ;//width/2 - x * trackHeight;
